...
class FetchPlaylist extends Command
{
     ...
     * @var string
     */
    protected $signature = 'fetch:playlist';
     ...
     * @var string
     */
    protected $description = 'Fetch all playlist from the brightcove';
    ...
    public function handle() {
        ...
        $this->info('Fetch Playlist Cron Job Started!');
        ...
        */
        if($totalPlaylist > 0) {
            ...
            $this->info('There are no playlist found using cURL from brightcove to add into database!');
        }

        $this->info('Fetch Playlist Cron Job Ended!');
    }
        ...
        if($checkPlaylistIDIfExists == 0) { 
            ...
            */
            ...
            if( $checkUpdate_at != $updated_at) {
                ...
                $this->info( 'There is update in playlist - '. $data['name']  );
                $this->deleteMetaPlaylistUsingID( $data['id'] );
                $this->info( 'Updating records for playlist - '. $data['name']  );
                $this->addPlaylist( $data );
                $this->addPlaylistMeta( $data );
            } else {
                $this->info( 'Playlist - '. $data['name'].' ( '. $data['id'] .' ) is Already Added into database!' );
            }
        }
    }
